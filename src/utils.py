...
def setup(cwd, sn_numbers):
    try:
        if not os.path.exists(f'{cwd}/data'):
            os.makedirs(f'{cwd}/data')
            print('Created data directory')

        if not os.path.exists(f'{cwd}/graphs'):
            os.makedirs(f'{cwd}/graphs')
            print('Created graphs directory')

        if not os.path.exists(f'{cwd}/processed_videos'):
            os.makedirs(f'{cwd}/processed_videos')
            print('Created processed videos directory')

        if not os.path.exists(f'{cwd}/uploaded_files'):
            os.makedirs(f'{cwd}/uploaded_files')
            print('Created uploaded files directory')

        if not os.path.exists(f'{cwd}/webpages'):
            os.makedirs(f'{cwd}/webpages')
            print('Created webpages directory')

        if not os.path.exists(f'{cwd}/uploaded_videos'):
            os.makedirs(f'{cwd}/uploaded_videos')
            print('Created uploaded videos directory')
        ...
        for sn in sn_numbers:
            ...
            if not os.path.exists(f'{cwd}/graphs/{sn}'):
                os.makedirs(f'{cwd}/data/{sn}')
                print(f'Created data/{sn} directory')
...
def extract_uploaded_files(cwd, uploaded_files, file_type):
    if file_type == 'log':
        ...
        directory_name = 'uploaded_files'
    elif file_type == 'video':
        supported_extensions = ['.mp4', '.h264']
        directory_name = 'uploaded_videos'
    else:
...
def clear_uploaded_files(cwd):
    try:
        for file in os.listdir(f'{cwd}/uploaded_files'):
            os.remove(f'{cwd}/uploaded_files/{file}')
...
def clear_uploaded_videos(cwd):
    try:
        for video in os.listdir(f'{cwd}/uploaded_videos'):
            os.remove(f'{cwd}/uploaded_videos/{video}')
        print('Uploaded videos cleared')

        for video in os.listdir(f'{cwd}/processed_videos'):
            if video.endswith('_graph_video.mp4') or video.endswith('_10_fps.mp4'):
                os.remove(f'{cwd}/processed_videos/{video}')
